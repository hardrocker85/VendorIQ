...
function optimizePrice(records) {
    ...
    if (priceStd < 2) {
        return {
            stop: true,
            warning: 'Try slightly different prices across days for better recommendations'
        };
    }
    ...
    // 5. Always run optimization on bounded price range
    const minPrice = Math.min(...prices);
    const maxPrice = Math.max(...prices);
    const lowerBound = Math.max(0.01, minPrice * 0.9);
    const upperBound = Math.max(lowerBound * 1.05, maxPrice * 1.1);
    const numPoints = 100;
    const step = (upperBound - lowerBound) / (numPoints - 1);

    const demand = (p) => Math.max(0, (a * p * p) + (b * p) + c);

    let optimalPrice = lowerBound;
    ...
    for (let i = 0; i < numPoints; i++) {
        ...
        const qPred = Math.max(demand(price), 0);
    ...
    const edgeBand = (upperBound - lowerBound) * 0.05;
    if (optimalPrice >= upperBound - edgeBand) optimalPrice *= 0.95;
    if (optimalPrice <= lowerBound + edgeBand) optimalPrice *= 1.05;

    const currentPrice = prices.reduce((s, v) => s + v, 0) / prices.length;
    let bestPrice = Math.round(optimalPrice);
    bestPrice = Math.max(lowerBound, Math.min(bestPrice, upperBound));

    // Optional stability boost
    let finalPrice = Math.round((0.7 * bestPrice) + (0.3 * currentPrice));
    finalPrice = Math.max(lowerBound, Math.min(finalPrice, upperBound));
    const finalDemand = Math.max(demand(finalPrice), 0);
    const finalProfit = (finalPrice - avgCost) * finalDemand;
...
function updateRecommendationPanel() {
    ...
    if (count < MIN_DATA_POINTS) {
        ...
        if (optResult.stop) {
            document.getElementById('rec-price').innerText = '₹--';
        ...
        document.getElementById('rec-price').innerText = formatCurr(optResult.optimalPrice);
